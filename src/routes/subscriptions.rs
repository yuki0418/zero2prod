...
pub async fn subscribe(
    ...
    if send_confirmation_email(&email_client, new_subscriber, &base_url.0, &subscriber_token)
        .await
        .is_err()
    {
        return HttpResponse::InternalServerError().finish();
...
#[tracing::instrument(name = "Store subscription token in the database", skip())]
